...
import numpy as np
import random
from dataset_txt import Data
from my_parser_txt import parse
from tqdm import tqdm
from model_txt import ReED
import os
import copy
from evaluate_txt import evaluate
...
ent_feats = dataset.ent_feats
rel_feats = dataset.rel_feats

my_model = ReED(hid_ent = args.dimension, hid_rel = args.dimension, dim_final = args.dimension, 
                 num_RAMPLayer = args.num_RAMPLayer, num_ent = len(ent_feats), num_rel = len(rel_feats), dim_in_ent = len(ent_feats[0]), dim_in_rel = len(rel_feats[0]),
...
for epoch in tqdm_bar:
    ...
    for batch, batch_label in zip(torch.split(train, batch_size), torch.split(label, batch_size)):
        optimizer.zero_grad()
        emb_ent, emb_rel = my_model(msg, ent_feats, rel_feats)
...
emp_loss = evaluate(my_model, ent_feats, rel_feats, msg, train_pos, train_neg, margin = args.margin)
exp_loss = evaluate(my_model, ent_feats, rel_feats, msg, torch.cat([train_pos, valid_pos, test_pos], dim = 0), torch.cat([train_neg, valid_neg, test_neg], dim = 0))
